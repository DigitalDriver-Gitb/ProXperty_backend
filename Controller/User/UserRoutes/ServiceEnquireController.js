import ServiceEnquire from "../../../models/Users/FormHandlermodel/ServiceEnqireModal.js";
export const createServiceEnquiry = async(req,res) =>{
    try{
        const {name,email,phone,serviceName,status} = req.body;
        const enquiry = new ServiceEnquire({
            name,
            email,
            phone,
            serviceName,
...
export const getServiceEnquiries = async(_req,res) =>{
    try{
        const enquiries = await ServiceEnquire.find().sort({ createdAt: -1 });
...
export const updateServiceEnquiryStatus = async (req, res) => {
  try {
    ...
    const enquiry = await ServiceEnquire.findByIdAndUpdate(
      id,
      { status },
      { new: true }
    );
